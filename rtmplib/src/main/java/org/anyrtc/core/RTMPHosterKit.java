...
public class RTMPHosterKit {
    ...
    public void SetVideoWHF(final int W,final int H ,final int F){
        m_Width = W;
        m_Height = H;
        m_Fps = F;
    }
        ...
        mExecutor.execute(new Runnable() {
            ...
            public void run() {
                if(mVideoCapturer != null) {
                    ...
                    nativeSetVideoCapturer(null, 0,0,0,0);
                    mVideoCapturer = null;
                }
                nativeDestroy();
            }
    ...
    public void SetVideoCapturer(final long renderPointer, final boolean front){
        mExecutor.execute(new Runnable() {
            ...
            public void run() {
                ...
                nativeSetVideoCapturer(mVideoCapturer, renderPointer,m_Width,m_Height,m_Fps);
            }
        });
    }
    ...
    public void StartRtmpStream(final String strUrl){
        mExecutor.execute(new Runnable() {
            @Override
            public void run() {
                nativeStartRtmpStream(strUrl,m_Width,m_Height,m_Fps);
            }
        });
    }
    ...
    private native void nativeSetVideoCapturer(VideoCapturer capturer, long nativeRenderer,int w,int h,int fps);
    private native void nativeStartRtmpStream(String strUrl,int w,int h,int fps);
