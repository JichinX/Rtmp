...
void RtmpHosterImpl::SetVideoCapturer(void* handle)
{
 ...
 void RtmpHosterImpl::SetVideoWHF(int w,int h,int fps)
{
      req_w_ = w;
	  req_h_ = h;
	v_width_ = w;
	v_height_ = h;
	 ...
	 else if((req_w_ <1000)&&(req_w_ >640))
	 {
		 bitrate = 768;
	 }
	 else
...
void RtmpHosterImpl::AddVideoCapturer_w(void* handle)
{
	...
	if (video_capturer_ != nullptr) {
...

		{
			...
			highest_asked_format.Construct(req_w_, req_h_, cricket::VideoFormat::FpsToInterval(v_fps), cricket::FourCC::FOURCC_NV12);
